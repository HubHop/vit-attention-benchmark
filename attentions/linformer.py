"""
    ...
    mainly modified from https://github.com/lucidrains/linformer/blob/master/linformer/linformer.py
"""
import math
import torch
...
class LinformerSelfAttention(nn.Module):
    def __init__(self, dim, lin_feat=256, num_heads=8, qkv_bias=False,
                 qk_scale=None, attn_drop=0., proj_drop=0., share_kv=False, seq_len=196):
        super().__init__()
        assert (dim % num_heads) == 0, 'dimension must be divisible by the number of heads'
        self.dim = dim
        self.seq_len = seq_len
        self.num_feats = lin_feat
...
if __name__ == '__main__':
    ...
    # special for linformer
    lin_feat = 128

    model = LinformerSelfAttention(dim=dim, lin_feat=lin_feat, num_heads=num_heads, qkv_bias=True)
